...
class BodySkeleton: Entity {
    ...
    required init(for bodyAnchor: ARBodyAnchor){
        ...
        for jointName in ARSkeletonDefinition.defaultBody3D.jointNames {
            ...
            case "left_shoulder_1_joint", "right_shoulder_1_joint":
                jointRadius *= 0.5
            case "jaw_joint", "chin_joint", "left_eye_joint", "right_eye_joint":
                jointRadius *= 0.2
                jointColor = .white
    ...
    private func createJoint(radius: Float, color: UIColor) -> Entity {
        ...
        // Using UnlitMaterial for a "glowing" effect
        var material = UnlitMaterial()
        material.color = .init(tint: color.withAlphaComponent(0.8), texture: nil)
    ...
    private func createBoneEntity(for skeletonBone: SkeletonBone, diameter: Float = 0.015) -> Entity {
        let mesh = MeshResource.generateBox(size: [diameter, diameter, skeletonBone.length], cornerRadius: diameter/2)
        var material = UnlitMaterial()
        material.color = .init(tint: .white.withAlphaComponent(0.5), texture: nil)
        let entity = ModelEntity(mesh: mesh, materials: [material])
        return entity
    }
}
